...
void main() {
  ...
  test(
    'first block is a LetterheadBlock when no logo (companyName non-empty)',
    () {
      const t = CoeTemplate();
  ...
  test('first block is a LetterheadBlock with logoBytes when logo set', () {
    ...
    final blocks = t.build(
      filled(logo: Uint8List.fromList(const [137, 80, 78, 71])),
    );
    expect(blocks.first, isA<LetterheadBlock>());
  ...
  test('certify paragraph uppercases company and position', () {
    const t = CoeTemplate();
    final blocks = t.build(filled());
    final certify = blocks.whereType<ParagraphBlock>().firstWhere(
      (b) => b.text.startsWith('This is to certify'),
    );
  ...
  test('issued paragraph uses ordinal day + place', () {
    const t = CoeTemplate();
    final blocks = t.build(filled());
    final issued = blocks.whereType<ParagraphBlock>().firstWhere(
      (b) => b.text.startsWith('Given this'),
    );
