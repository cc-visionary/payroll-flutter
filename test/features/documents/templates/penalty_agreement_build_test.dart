...
void main() {
  group('PenaltyAgreementTemplate.build', () {
    test('lays out letterhead → meta → schedule → signatures', () {
      ...
      expect(
        blocks.whereType<LetterMetaBlock>().single.subject,
    ...
    test(
      'builds with only HR signed — employee gets a blank wet-sign line',
      () async {
    ...
    test(
      'a corrupt base64 signature degrades to a blank line, not a crash',
      () {
    ...
    test('emptyInputs still produce renderable blocks', () {
      final blocks = const PenaltyAgreementTemplate().build(
    ...
    test('remarks render only when present', () {
      ...
      final with_ = const PenaltyAgreementTemplate().build(
        _seed().copyWith(remarks: 'Starts on the Aug 15 cutoff.'),
      );
