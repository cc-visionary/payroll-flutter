...
void main() {
  group('EmploymentContractInputs.fromJson', () {
    test('round-trips toJson, including nested lists, training wage and dates',
        () {
  ...
  group('blocksForSavedDocument', () {
    test('rebuilds employment-contract blocks from saved generation_options',
        () {
      final options = {
        ..._sample().toJson(),
  ...
  group('every registered template is re-renderable', () {
    ...
    for (final t in kTemplates) {
      test('rebuilds ${t.id} blocks from emptyInputs via the central switch',
          () {
        final options = {
          ...t.emptyInputs().toJson(),
          '__template_id': t.id,
        };
