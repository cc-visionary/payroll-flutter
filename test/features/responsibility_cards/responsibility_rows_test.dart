...
void main() {
  test('responsibilitiesFromTaskRows groups + orders by sort, not name', () {
    ...
    expect(out.map((a) => a.area).toList(), [
      'Alpha',
      'Beta',
    ]); // area_sort order
    expect(out.first.tasks, [
      'Zebra task',
      'Apple task',
    ]); // task_sort, NOT alphabetical
    expect(out.length, 2); // blank-area row skipped
  });

  test(
    'diffResponsibilities updates renames by id (preserves costing) and assigns sorts',
    () {
      ...
      final d = diffResponsibilities(
        draft: [
          (
            area: 'Alpha',
            tasks: [
              RespDraft(id: 't1', name: 'New name'),
              RespDraft(id: null, name: 'Fresh'),
            ],
          ),
        ],
      ...
      expect(
        d.updates.single['name'],
        'New name',
      ); // rename = UPDATE, keeps hours
    ...
    test('an update carries role_scorecard_id so the row actually joins', () {
      ...
      final d = diffResponsibilities(
        draft: [
          (
            area: 'Setup',
            tasks: [RespDraft(id: 'legacy-1', name: 'SD card flashing')],
          ),
        ],
        existingRows: const [],
        cardId: 'rs1',
      ...
      expect(
        d.inserts,
        isEmpty,
        reason: 'it exists already — adopt, do not duplicate',
      );
    ...
    test('adopting does not delete rows already on the card', () {
      final d = diffResponsibilities(
        ...
        existingRows: const [
          {
            'id': 'own-1',
            'responsibility_area': 'Setup',
            'name': 'Existing responsibility',
          },
        ],
        cardId: 'rs1',
        companyId: 'c',
