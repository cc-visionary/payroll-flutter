...
void main() {
  ...
  group('lastReleasedPeriodEnd', () {
    test('scopes to the company (ignores another company\'s newer release)', () {
      // Newest-first: company A released more recently than company B.
      final runs = [
        _run(company: 'A', status: 'RELEASED', freq: 'SEMI_MONTHLY', end: '2026-07-02', payDate: '2026-07-02'),
        _run(company: 'B', status: 'RELEASED', freq: 'SEMI_MONTHLY', end: '2026-06-15', payDate: '2026-06-15'),
      ];
      // For company B we must get B's end, NOT A's more-recent one.
      expect(
        lastReleasedPeriodEnd(runs, companyId: 'B', frequency: 'SEMI_MONTHLY'),
        DateTime(2026, 6, 15),
      );
    });
    ...
    test('prefers the same frequency, else falls back to any', () {
      final runs = [
        _run(company: 'A', status: 'RELEASED', freq: 'MONTHLY', end: '2026-07-01', payDate: '2026-07-05'),
        _run(company: 'A', status: 'RELEASED', freq: 'SEMI_MONTHLY', end: '2026-06-15', payDate: '2026-06-20'),
      ];
    ...
    test('ignores non-RELEASED runs; null when the company has no releases', () {
      final runs = [
        _run(company: 'A', status: 'DRAFT', freq: 'SEMI_MONTHLY', end: '2026-07-02', payDate: '2026-07-02'),
        _run(company: 'A', status: 'RELEASED', freq: 'SEMI_MONTHLY', end: '2026-06-15', payDate: '2026-06-15'),
      ];
      expect(
        lastReleasedPeriodEnd(runs, companyId: 'A', frequency: 'SEMI_MONTHLY'),
      ...
      expect(
        lastReleasedPeriodEnd(runs, companyId: 'Z', frequency: 'SEMI_MONTHLY'),
