...
WpTask _task(String id, String? cardId, {String status = 'ACTIVE'}) => WpTask(
  id: id,
  companyId: 'c',
  name: id,
  roleScorecardId: cardId,
...
void main() {
  group('monthlyCostPerHolder mirrors the payroll wage-type switch', () {
    test('DAILY x 26 working days', () {
      expect(
        monthlyCostPerHolder(_card('a', 'A', base: '1076.92')),
    ...
    test('MONTHLY is already monthly', () {
      expect(
        monthlyCostPerHolder(
          _card('a', 'A', base: '30000', wageType: 'MONTHLY'),
        ),
        Decimal.parse('30000'),
      );
    });
    ...
    test('no base salary -> null, never zero', () {
      expect(
        monthlyCostPerHolder(_card('a', 'A')),
        isNull,
  ...
  group('rollup', () {
    test(
      'sums the role\'s whole responsibility set, undivided by headcount',
      () {
        final rows = _build(
        ...
        expect(
          r.hoursPerMonth,
          100.0,
          reason: 'the role\'s work is 100h either way',
        );
        expect(r.holders, 2);
    ...
    test('an uncosted role reports unknown load, not 0%', () {
      ...
      expect(
        r.loadFraction,
        isNull,
        reason: '0% would read as "this role is idle"',
      );
    ...
    test(
      'an ARCHIVED task on a card is not counted in that card\'s rollup row',
      () {
        final rows = _build(
          cards: [_card('rs1', 'Ops', base: '1000')],
          employees: [_emp('e1', 'rs1')],
        ...
        expect(
          ...
          40.0,
          reason:
              'the archived task\'s hours must not leak into the role\'s workload',
        );
        expect(
          r.fullyCosted,
          isTrue,
          reason:
              'fullyCosted must not be flipped by an archived, uncounted task',
        );
      },
    );
