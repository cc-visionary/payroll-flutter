...
WpTaskAssignment _pa(String emp, double pct, {String role = 'PRIMARY'}) =>
    WpTaskAssignment(
      id: 'a',
      companyId: 'c',
      taskId: 't',
...
WpTaskAssignment _ca(String card, double pct, {String role = 'PRIMARY'}) =>
    WpTaskAssignment(
      id: 'a',
      companyId: 'c',
      taskId: 't',
...
void main() {
  test('move override gives 100% to the moved person', () {
    ...
    expect(r.shares, [
      (employeeId: 'x', hours: 40.0, derived: false, holderCount: 1),
    ]);
    expect(r.unattributed, 0);
  });

  test(
    'two person assignments split by pct; Σ100 leaves nothing unattributed',
    () {
      final r = attributeTask(
        hours: 100,
        task: _task,
  ...
  test('fallback: no assignments + owner -> owner full', () {
    ...
    expect(r.shares, [
      (employeeId: 'o', hours: 40.0, derived: false, holderCount: 1),
    ]);
    expect(r.unattributed, 0);
  });
  ...
  test(
    'an uncosted (0h) task with a person assignment still identifies the carrier',
    () {
      final r = attributeTask(
        hours: 0,
        task: _task,
        assignments: [_pa('a', 100)],
        holdersOf: _holders(const []),
      );
      expect(r.shares, [
        (employeeId: 'a', hours: 0.0, derived: false, holderCount: 1),
      ]);
      expect(r.unattributed, 0);
    },
  );
  ...
  test(
    'an uncosted (0h) task with the owner fallback still identifies the owner',
    () {
      const owned = WpTask(
        id: 't',
        companyId: 'c',
      ...
      expect(r.shares, [
        (employeeId: 'o', hours: 0.0, derived: false, holderCount: 1),
      ]);
      expect(r.unattributed, 0);
    },
  );
