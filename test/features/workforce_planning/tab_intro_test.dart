...
void main() {
  testWidgets('the purpose is always visible, the detail is not', (tester) async {
    await tester.pumpWidget(_host(const TabIntro(
      purpose: 'Move work until nobody is over capacity.',
      details: [WpGlossary.load],
    )));
    expect(find.text('Move work until nobody is over capacity.'), findsOneWidget);
    expect(find.textContaining('÷ that person', findRichText: true), findsNothing,
  ...
  testWidgets('expanding reveals the terms and collapsing hides them again',
    ...
    expect(find.textContaining('whoever holds its role card', findRichText: true),
        findsOneWidget);

    await tester.tap(find.text('Hide'));
    await tester.pumpAndSettle();
    expect(find.textContaining('Over 100% = Over', findRichText: true), findsNothing);
  });

  testWidgets('with no details there is nothing to expand', (tester) async {
    await tester.pumpWidget(_host(const TabIntro(purpose: 'Just a line', details: [])));
  ...
  test('glossary entries are non-empty and distinct', () {
    ...
    for (final e in all) {
      ...
      expect(e.meaning.trim().length, greaterThan(20),
