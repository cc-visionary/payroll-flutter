...
void main() {
  ...
  test('finds the spec\'s motivating duplicate: a manager retyping '
    ...
    final m = findSimilarAccountabilities(
      typed: 'Pack and dispatch orders',
      all: all,
    );
    expect(m, isNotEmpty);
  ...
  test('ignores archived and expectation rows', () {
    expect(
      findSimilarAccountabilities(
        typed: 'Old packing flow',
        all: all,
      ).any((m) => m.task.id == '3'),
      isFalse,
    );
  ...
  test('unrelated names sharing no words score 0 and do not match', () {
    expect(
      findSimilarAccountabilities(typed: 'Design social ads', all: all),
      isEmpty,
    );
    expect(
      findSimilarAccountabilities(
        typed: 'Reconcile bank statements',
        all: all,
      ).any((m) => m.task.id == '1'),
      isFalse,
    );
    final adsVsBank = [
      _t('a', 'Design social ads'),
      _t('b', 'Reconcile bank statements'),
    ];
    expect(
      findSimilarAccountabilities(
        typed: 'Design social ads',
        all: adsVsBank,
      ).any((m) => m.task.id == 'b'),
      isFalse,
    );
  ...
  test('respects the limit', () {
    final many = [
      for (var i = 0; i < 10; i++) _t('$i', 'Pack orders daily $i'),
    ];
    expect(
      findSimilarAccountabilities(
        typed: 'Pack orders daily',
        all: many,
        limit: 3,
      ).length,
      3,
