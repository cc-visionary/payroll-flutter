...
void main() {
  ...
  testWidgets('descendants are visible by default, at every depth', (tester) async {
    final emps = {
      'a': _e('a', 'A', 'One', null),
      'b': _e('b', 'B', 'Two', 'a'),
      'c': _e('c', 'C', 'Three', 'b'),
    ...
    await tester.pumpAndSettle();
    expect(find.text('C Three'), findsOneWidget, reason: 'grandchild must not start collapsed');
  });

  testWidgets('collapsing a manager hides their subtree', (tester) async {
    final emps = {'a': _e('a', 'A', 'One', null), 'b': _e('b', 'B', 'Two', 'a')};
    await tester.pumpWidget(_host(OrgChartView(
  ...
  testWidgets('a person with no employee record still renders as its id', (tester) async {
    await tester.pumpWidget(_host(const OrgChartView(
