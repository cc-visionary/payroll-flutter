...
Widget _host({
  ...
  overrides: [
    ...
    wpPersonLoadsProvider.overrideWith(
      (ref) async => [for (final e in employees) _cap(e.id, 160)],
    ),
...
void main() {
  testWidgets('ranks people by load and flags the one over capacity', (
    tester,
  ) async {
  ...
  testWidgets('the busiest person is selected first and their work is listed', (
    ...
    await tester.pumpWidget(
      _host(employees: _people, tasks: _tasks, computed: _computed),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('dragging a task onto a person drafts a move without saving it', (
    ...
    expect(
      repo.reassigned,
      isEmpty,
      reason: 'a draft must not touch the database',
    );
  ...
  testWidgets('Reset discards the drafts', (tester) async {
    ...
    expect(
      find.text('131%'),
      findsOneWidget,
      reason: 'back to the original split',
    );
  });
  ...
  testWidgets(
    'a shared responsibility warns that moving it hands over the whole task',
    (tester) async {
      await tester.pumpWidget(
        _host(
          employees: [
      ...
      expect(
        find.text('20.0h'),
        findsOneWidget,
        reason: 'each holder carries half',
      );
    },
  );

  testWidgets('unattributed work is surfaced, not silently dropped', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
  ...
  testWidgets('legacy reference rows are not reported as unassigned debt', (
    ...
    await tester.pumpAndSettle();
    expect(
      find.textContaining('800.0h unassigned'),
      findsNothing,
  ...
  testWidgets('a person with uncosted work is flagged as understated', (
    ...
    expect(
      find.textContaining('1 of 2 uncosted — understated'),
      findsOneWidget,
      ...
      _host(
        ...
        tasks: const [
          WpTask(
            id: 't1',
            companyId: 'c',
            name: 'Real costed work',
            ownerEmployeeId: 'e1',
          ),
          WpTask(
            id: 't2',
            companyId: 'c',
            name: 'Ensure procedures are followed',
            ownerEmployeeId: 'e1',
            isExpectation: true,
          ),
        ],
  ...
  testWidgets('the tab says what it is for, with detail on request', (
    ...
    await tester.pumpAndSettle();
    expect(
      find.textContaining('Move work until nobody is over capacity'),
      findsOneWidget,
    );
    expect(
      find.textContaining('struck through', findRichText: true),
      findsNothing,
    );
    ...
    await tester.pumpAndSettle();
    expect(
      find.textContaining('Nothing saves until Apply', findRichText: true),
      findsOneWidget,
    );
    expect(
