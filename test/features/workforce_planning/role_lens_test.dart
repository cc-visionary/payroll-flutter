...
RoleScorecard _card(String id, String title, {String? base}) => RoleScorecard(
      ...
      wageType: 'DAILY', workHoursPerDay: 8, workDaysPerWeek: 'MON_FRI',
      isActive: true, effectiveDate: DateTime(2026, 1, 1),
    );

Employee _emp(String id, String name, String? cardId) => Employee(
      id: id, companyId: 'c', employeeNumber: id, firstName: name, lastName: 'X',
...
WpPersonLoad _load(String id) => WpPersonLoad(
    employeeId: id, companyId: 'c', capacityHours: 160, growthMultiplier: 1);
    ...
    ProviderScope(
      overrides: [
        ...
        wpPersonLoadsProvider
            .overrideWith((ref) async => [for (final e in employees) _load(e.id)]),
...
void main() {
  ...
  testWidgets('shows hours, load, cost and cost-per-hour for a costed role',
    ...
    await tester.pumpWidget(_host(
      cards: [_card('rs1', 'Operations Manager', base: '1000')],
      employees: [_emp('e1', 'Jeremy', 'rs1')],
      tasks: const [WpTask(id: 't1', companyId: 'c', name: 'Pack', roleScorecardId: 'rs1')],
      computed: const [
  ...
  testWidgets('an uncosted role shows dashes, never a confident 0%', (tester) async {
    await tester.pumpWidget(_host(
    ...
    expect(find.text('0/2'), findsWidgets, reason: 'costed/total is spelled out');
  ...
  testWidgets('warns how many responsibilities are uncosted', (tester) async {
    ...
    await _toRoleLens(tester);
    expect(find.textContaining('1 of 2 responsibilities are not costed'), findsOneWidget);
  });
  ...
  testWidgets('a vacant role shows no capacity and no cost', (tester) async {
    await tester.pumpWidget(_host(
      ...
      tasks: const [WpTask(id: 't1', companyId: 'c', name: 'A', roleScorecardId: 'rs1')],
      computed: const [
    ...
    expect(find.text('₱0'), findsWidgets, reason: 'nobody is being paid for it');
  });

  testWidgets('the total row does not invent a company-wide load percentage',
      (tester) async {
    await tester.pumpWidget(_host(
      cards: [_card('rs1', 'A', base: '1000'), _card('rs2', 'B', base: '500')],
      employees: [_emp('e1', 'X', 'rs1'), _emp('e2', 'Y', 'rs2')],
      tasks: const [WpTask(id: 't1', companyId: 'c', name: 'T', roleScorecardId: 'rs1')],
      computed: const [
        WpTaskComputed(taskId: 't1', companyId: 'c', hoursPerMonthBase: 80),
      ],
    ));
