...
void main() {
  ...
  group('groupTasks', () {
    test(
      'buckets a card-linked task under its card, ordered areas/tasks by sort not name',
      () {
        final tasks = [
          WpTask.fromRow({
            'id': 't1',
            'company_id': 'c',
            'name': 'Zebra task',
            'role_scorecard_id': 'rc1',
            'responsibility_area': 'B area',
            'area_sort': 0,
            'task_sort': 1,
          }),
          WpTask.fromRow({
        ...
        expect(cg.areas[0].tasks.map((t) => t.name).toList(), [
          'Apple task',
          'Zebra task',
        ]);
      },
    ...
    test(
      'task_sort order beats alphabetical order (not merely consistent with it)',
      () {
        final tasks = [
          WpTask.fromRow({
            'id': 't1',
        ...
        final groups = groupTasks(tasks, [_card('rc1', 'Ops Lead')]);
        final names = groups.cardGroups.single.areas.single.tasks
            .map((t) => t.name)
            .toList();
    ...
    test(
      'area order uses the MIN area_sort across a possibly-inconsistent set of rows',
      () {
        final tasks = [
          WpTask.fromRow({
            'id': 't1',
          ...
          WpTask.fromRow({
            ...
            'name': 'x3',
            'role_scorecard_id': 'rc1',
            'responsibility_area': 'Early',
            'area_sort': 2,
            'task_sort': 0,
          }),
        ];
        final groups = groupTasks(tasks, [_card('rc1', 'Ops Lead')]);
        expect(groups.cardGroups.single.areas.map((a) => a.area).toList(), [
          'Late',
          'Early',
        ]);
      },
    ...
    test(
      'a task with a null/blank responsibility area is not dropped, grouped as Unspecified',
      () {
        final tasks = [
          WpTask.fromRow({
            'id': 't1',
        ...
        final groups = groupTasks(tasks, [_card('rc1', 'Ops Lead')]);
        expect(
          groups.cardGroups.single.areas.single.area,
          unspecifiedAreaLabel,
        );
    ...
    test(
      'a task pointing at a card outside the active-only fetch still gets a heading',
      () {
        ...
        final tasks = [
          const WpTask(
            ...
            companyId: 'c',
            name: 'orphaned',
            roleScorecardId: 'rc-gone',
          ),
        ];
        final groups = groupTasks(
          tasks,
          const [],
        ); // rc-gone not in the fetched cards
        expect(groups.cardGroups, hasLength(1));
    ...
    test(
      'only card ids with >=1 task get a heading (no empty-card headings)',
      () {
        final tasks = [
          const WpTask(
            id: 't1',
    ...
    test(
      'unattributed bucket is the TRUE complement of buckets 1+2, not "no owner"',
      () {
    ...
    test(
      'roleScorecardId takes priority over externalRef (bucket 1 before bucket 2)',
      () {
        final tasks = [
          const WpTask(
            id: 't1',
    ...
    test('known-card groups follow the (already job-title-sorted) cards order; '
      ...
      final tasks = [
        const WpTask(
          ...
          companyId: 'c',
          name: 'a',
          roleScorecardId: 'rc-unknown-2',
        ),
        const WpTask(
          id: 't2',
          companyId: 'c',
          name: 'b',
          roleScorecardId: 'rc-unknown-1',
        ),
        const WpTask(
          id: 't3',
          companyId: 'c',
  ...
  group('isTaskNotCosted', () {
    test(
      'true when neither times nor minutes is set (freshly-promoted responsibility)',
      () {
    ...
    test(
      'rate-sourced minutes counts as having minutes when rateId is set',
      () {
        const t = WpTask(
          id: 't1',
          companyId: 'c',
    ...
    test(
      'a direct-hours task (hoursPerMonth set, no times/minutes) is costed',
      () {
        const t = WpTask(
          id: 't1',
          companyId: 'c',
  ...
  group('taskHours', () {
    ...
    test(
      'a direct hoursPerMonth wins outright, ignoring times/minutes entirely',
      () {
        const t = WpTask(
          id: 't1',
          companyId: 'c',
  ...
  group('resolveEffectiveOwner', () {
    test('explicit owner: shows their name, not derived', () {
      ...
      final owner = resolveEffectiveOwner(
        task: t,
        employeeNameById: const {'e1': 'Marvin Ong'},
        employees: const [],
      );
    ...
    test(
      'explicit owner not resolvable in the active roster falls back to Unassigned, still not derived',
      () {
        const t = WpTask(
          id: 't1',
          companyId: 'c',
    ...
    test(
      'zero role holders: Unassigned, NOT derived (nothing was actually derived)',
      () {
        const t = WpTask(
          id: 't1',
          companyId: 'c',
  ...
  group('groupTasks task ordering tie-break', () {
    test('equal task_sort within an area is tie-broken by id for a '
      ...
      final groups = groupTasks(tasks, [_card('rc1', 'Ops Lead')]);
      final ids = groups.cardGroups.single.areas.single.tasks
          .map((t) => t.id)
          .toList();
