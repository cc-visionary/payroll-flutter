...
void main() {
  final kpis = [
    _k(
      ...
      measure: 'errors ÷ orders',
    ),
    _k(
      '2',
      'On-Time Dispatch',
      category: 'Timeliness & Delivery',
      measure: 'within cutoff',
    ),
  ...
  group('filtering', () {
    test('deactivated KPIs are hidden unless asked for', () {
      expect(
        applyKpiFilter(kpis, const KpiFilter(), assigned).map((k) => k.id),
        ['1', '2', '3'],
      ...
      expect(
        applyKpiFilter(
          kpis,
          const KpiFilter(showInactive: true),
          assigned,
        ).map((k) => k.id),
        ['1', '2', '3', '4'],
      );
    ...
    test('search covers name, category and measurement', () {
      List<String> ids(String q) => applyKpiFilter(
        kpis,
        KpiFilter(query: q),
        assigned,
      ).map((k) => k.id).toList();
    ...
    test('assignment filter separates tracked from measuring-nobody', () {
      expect(
        applyKpiFilter(
          kpis,
          const KpiFilter(assignment: true),
          assigned,
        ).map((k) => k.id),
        ['1'],
      );
      expect(
        applyKpiFilter(
          kpis,
          const KpiFilter(assignment: false),
          assigned,
        ).map((k) => k.id),
        ['2', '3'],
    ...
    test('category filter, and filters combine', () {
      expect(
        applyKpiFilter(
          kpis,
          const KpiFilter(category: 'Quality & Accuracy'),
          assigned,
        ).map((k) => k.id),
        ['1'],
      );
  ...
  group('stats', () {
    test('counts actives, coverage, and distinct people', () {
      ...
      expect(
        s.active,
        3,
        reason: 'the deactivated one is not part of coverage',
      );
      expect(s.assigned, 1);
  ...
  group('departments', () {
    ...
    test('a missing or unknown department id reads as "No department"', () {
      ...
      expect(
        kpiDepartmentOf(withDepts[3], _deptNames),
        kNoDepartment,
    ...
    test('grouping nests department -> category -> KPIs', () {
      ...
      expect(g['Operations']!['Quality']!.map((k) => k.name), [
        'Order Accuracy',
      ]);
      expect(
        g[kNoDepartment]!['Quality']!.map((k) => k.name),
    ...
    test('filtering by department', () {
      expect(
        applyKpiFilter(
          withDepts,
          const KpiFilter(department: 'Operations'),
          const {},
      ...
      expect(
        applyKpiFilter(
          withDepts,
          const KpiFilter(department: kNoDepartment),
          const {},
          departmentNameById: _deptNames,
        ).map((k) => k.id),
        ['3', '4'],
