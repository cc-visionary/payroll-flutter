...
EmployeePayrollInput _employee({
  ...
  for (var day = 1; day <= 22; day++) {
    attendance.add(AttendanceDayInput(
      ...
      leaveIsPaid: false,
      dailyRateOverride:
          (overrideBefore != null && day < switchDay) ? overrideBefore : null,
    ));
  }
...
void main() {
  ...
  test('Test A — no override: pro-rated tax uses ACTUAL pay, below the flat run',
      () {
    final prorated = _run(_employee(
    ...
    expect(prorated.withholdingTax > Decimal.zero, isTrue,
        reason: 'pro-rated withholding must be positive (raise salary if not)');
    expect(flat.withholdingTax > Decimal.zero, isTrue,
    ...
    expect(prorated.withholdingTax < flat.withholdingTax, isTrue,
        reason:
            'pro-rated actual pay is lower, so its withholding must be lower');
  });

  test(
      'Test B — statutoryOverride: tax + statutory ignore per-day rates '
      '(hard invariant)', () {
    final override =
        StatutoryOverride(baseRate: _d('20000'), wageType: WageType.MONTHLY);
    ...
    final flat = _run(_employee(
        baseRate: _d('38000'), statutoryOverride: override));

    expect(prorated.withholdingTax, flat.withholdingTax,
        reason: 'declared-wage override fixes the tax basis; per-day rates '
            'must not move withholding');
    expect(prorated.sssEe, flat.sssEe, reason: 'SSS must ignore per-day rates');
    expect(prorated.philhealthEe, flat.philhealthEe,
        reason: 'PhilHealth must ignore per-day rates');
    expect(prorated.pagibigEe, flat.pagibigEe,
        reason: 'Pag-IBIG must ignore per-day rates');
  });

  test('Test C — no override: statutory contributions ignore per-day rates', () {
    final prorated = _run(_employee(
        baseRate: _d('38000'), overrideBefore: oldRate, switchDay: 17));
    final flat = _run(_employee(baseRate: _d('38000')));
