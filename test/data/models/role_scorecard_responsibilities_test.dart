...
void main() {
  test(
    ...
    () {
        ...
        baseRow(
          ...
          wpTasks: [
            ...
            },
            {
              'id': 't4',
              'name': 'Bravo task',
              'responsibility_area': 'Alpha Area',
              'area_sort': 1,
              'task_sort': 1,
            },
            {
      ...
      expect(contractResponsibilities.map((c) => c.area).toList(), [
        'Zulu Area',
        'Alpha Area',
      ]);
  ...
  test(
    'falls back to the legacy key_responsibilities JSON when wp_tasks is absent',
    () {
      final card = RoleScorecard.fromRow(
        baseRow(
          keyResponsibilities: [
            {
              'area': 'Sales',
              'tasks': ['Greet customers', 'Process returns'],
            },
          ],
        ),
      );
  ...
  test('an empty wp_tasks embed is authoritative — does NOT fall back to stale '
  ...
  test(
    'toUpsertPayload writes key_responsibilities as an empty array (NOT NULL, read-only)',
    () {
      final card = RoleScorecard.fromRow(
        baseRow(
          keyResponsibilities: [
  ...
  group('ANNEX A GATE (Risk #2) — a shared accountability APPENDS, it never '
    ...
    // added afterward.
    Map<String, dynamic> authoredRow() => baseRow(
      wpTasks: [
        {
          'id': 't3',
          'name': 'Beta task',
    ...
    test('authored responsibilities are byte-identical before and after an '
      ...
      expect(
        ...
        reason:
            'Annex A prefill: authored area order/wording unchanged by the union',
      );
      for (var i = 0; i < beforeContract.length; i++) {
        expect(
          afterContract[i].tasks,
          beforeContract[i].tasks,
          reason:
              'Annex A prefill: authored task order/wording unchanged at area $i',
        );
      }
    ...
    test('a task with no responsibility_area lands in a trailing "Shared" '
    ...
    test('a task both authored AND assigned to the SAME card (e.g. its PRIMARY '
      ...
      expect(
        extra,
        isEmpty,
        reason: 'self-authored task must not become a "shared" row',
      );
    ...
    test('multiple shared tasks are grouped/ordered by area_sort/task_sort '
      ...
      final sharedTasks = const [
        WpTask(
          id: 's-2',
          companyId: 'co-1',
          name: 'Second shared task',
          roleScorecardId: 'other-card',
          responsibilityArea: 'Onboarding',
          areaSort: 1,
          taskSort: 0,
        ),
        WpTask(
          id: 's-1',
          companyId: 'co-1',
          name: 'First shared task',
          roleScorecardId: 'other-card',
          responsibilityArea: 'Compliance',
          areaSort: 0,
          taskSort: 0,
        ),
      ];
      ...
      final trailing = after.responsibilities.skip(
        before.responsibilities.length,
      );
      expect(trailing.map((a) => a.area).toList(), [
        'Compliance',
        'Onboarding',
      ]);
    });
  ...
  group('owned-task append primitives', () {
    test('fallbackArea labels area-less tasks; default stays Shared', () {
      ...
      expect(
        responsibilitiesFromAssignedTasks('card-1', [
          noArea,
        ], fallbackArea: 'Additional Responsibilities').single.area,
    ...
    group('dedupeAppendedAreas', () {
      const existing = [
        ResponsibilityArea(
          area: 'Ops',
          tasks: ['Pack orders', 'Book couriers'],
        ),
      ];

      test(
        'drops case-insensitive/whitespace name collisions, prunes empty areas',
        () {
      ...
      test('keeps non-colliding tasks in order, dedupes within appended', () {
        final out = dedupeAppendedAreas(existing, const [
          ResponsibilityArea(
            area: 'Finance',
            tasks: ['Reconcile Xendit', 'Book couriers'],
          ),
      ...
      test(
        'empty appended → empty; empty existing keeps appended verbatim',
        () {
