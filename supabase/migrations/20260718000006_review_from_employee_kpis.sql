-- Phase 2 of KPI tracking, follow-up: generate_employee_review now snapshots
-- the employee's assigned KPI subset (employee_kpis) intersected with the
-- current role card, falling back to the full role set when the employee has
-- no assignment rows for that role's KPIs.
