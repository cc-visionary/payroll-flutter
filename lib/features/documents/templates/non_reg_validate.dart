...
List<ValidationError> validateNonReg(NonRegInputs i) {
  ...
  if (i.salutationName.trim().isEmpty) {
    errs.add(
      const ValidationError('salutationName', 'Salutation cannot be empty.'),
    );
  }
  ...
  if (i.findings.isEmpty) {
    errs.add(
      const ValidationError('findings', 'Add at least one finding section.'),
    );
  }
  for (var fi = 0; fi < i.findings.length; fi++) {
    ...
    if (f.title.trim().isEmpty) {
      errs.add(
        ValidationError('findings[$fi].title', 'Finding title is required.'),
      );
    }
    if (f.standard.trim().isEmpty) {
      errs.add(
        ValidationError('findings[$fi].standard', 'Standard body is required.'),
      );
    }
    if (f.finding.trim().isEmpty) {
      errs.add(
        ValidationError('findings[$fi].finding', 'Finding body is required.'),
      );
    }
