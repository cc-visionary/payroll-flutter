...
List<ValidationError> validateEmploymentContract(EmploymentContractInputs i) {
  ...
  req(
    i.employeeAddress.trim().isEmpty,
    'employeeAddress',
  ...
  req(
    i.representativeName.trim().isEmpty,
    'representativeName',
    'Company representative name is required.',
  );
  req(
    i.representativeRole.trim().isEmpty,
    'representativeRole',
  ...
  if (i.probationStart == null) {
    errs.add(
      const ValidationError(
        'probationStart',
        'Probationary start date is required.',
      ),
    );
  }
  if (i.probationEnd == null) {
    errs.add(
      const ValidationError(
        'probationEnd',
        'Probationary end date is required.',
      ),
    );
  }
  ...
  if (i.nonCompeteMonths <= 0) {
    errs.add(
      const ValidationError(
        'nonCompeteMonths',
        'Non-compete period must be greater than 0.',
      ),
    );
  }
  ...
  if (i.responsibilities.isEmpty) {
    errs.add(
      const ValidationError(
        'responsibilities',
        'Add at least one responsibility area (Annex A).',
      ),
    );
  }
  for (var ri = 0; ri < i.responsibilities.length; ri++) {
    ...
    if (r.area.trim().isEmpty) {
      errs.add(
        ValidationError(
          'responsibilities[$ri].area',
          'Responsibility area is required.',
        ),
      );
    }
