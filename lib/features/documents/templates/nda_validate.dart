...
List<ValidationError> validateNda(NdaInputs i) {
  ...
  req(i.employeePosition.trim().isEmpty, 'employeePosition',
      'Position is required.');
  req(i.employeeHomeAddress.trim().isEmpty, 'employeeHomeAddress',
      'Home address is required.');
  req(i.companyName.trim().isEmpty, 'companyName',
      'Company name is required.');
  req(i.companyAddress.trim().isEmpty, 'companyAddress',
      'Company address is required.');
  req(i.authorizedSignatoryName.trim().isEmpty, 'authorizedSignatoryName',
      'Authorized signatory name is required.');
  if (i.effectiveDate == null) {
    errs.add(const ValidationError('effectiveDate', 'Effective date is required.'));
  }
  return errs;
}
