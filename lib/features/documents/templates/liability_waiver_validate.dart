...
List<ValidationError> validateLiabilityWaiver(LiabilityWaiverInputs i) {
  ...
  if (i.outingDate == null) {
    errors.add(const ValidationError('outingDate', 'Outing date is required.'));
  }
  if (i.outingLocation.trim().isEmpty) {
    errors.add(
      const ValidationError('outingLocation', 'Outing location is required.'),
    );
  }
  if (i.signingPlace.trim().isEmpty) {
    errors.add(
      const ValidationError('signingPlace', 'Place of signing is required.'),
    );
  }
  return errors;
}
