...
/// Every task attributed to one person: explicitly-owned tasks, PLUS the
/// unowned tasks on their role card (split across that card's ACTIVE holders).
///
/// This is the single Dart-side mirror of `wp_person_load`'s attribution rule.
/// Screens that list "this person's tasks" MUST use it rather than filtering on
/// `ownerEmployeeId` alone — a person's task list sitting under a load% derived
/// from the view would otherwise contradict it (e.g. "Load: 74%" above "No
/// owned tasks.") the moment any role-card responsibility is costed.
List<AttributedTask> tasksForPerson({
  ...
  int? cachedHolders;
  int holders() => cachedHolders ??= employees
      .where((e) =>
          e.roleScorecardId == roleScorecardId && e.employmentStatus == 'ACTIVE')
      .length;

  final out = <AttributedTask>[];
  for (final t in allTasks) {
    if (t.ownerEmployeeId == employeeId) {
      out.add(AttributedTask(task: t, derived: false, holderCount: 1));
    } else if (t.ownerEmployeeId == null &&
        roleScorecardId != null &&
        t.roleScorecardId == roleScorecardId) {
      final n = holders();
      if (n > 0) {
        out.add(AttributedTask(task: t, derived: true, holderCount: n));
      }
    }
  }
  return out;
