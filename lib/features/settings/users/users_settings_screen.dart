...
class UsersSettingsScreen extends ConsumerWidget {
  ...
  Future<void> _openAddDialog(BuildContext context, WidgetRef ref) async {
    await showDialog(
      context: context,
      builder: (_) =>
          _AddUserDialog(onCreated: () => ref.invalidate(managedUsersProvider)),
    );
  }
}
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  user.email +
                      (user.linkedEmployeeName == null
                          ? ' · no employee link'
                          : ' · linked: ${user.linkedEmployeeName}'),
          ...
          PopupMenuButton<_UserAction>(
            ...
            itemBuilder: (_) => [
              ...
              const PopupMenuItem(
                value: _UserAction.linkEmployee,
                child: Text('Link / unlink employee'),
              ),
              if (user.isInactive)
                const PopupMenuItem(
    ...
    if (_createdPassword != null) {
      return AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              '${_email.text} can now sign in. The temporary password is shown below — copy it now, it will not be shown again.',
            ),
                ...
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () => Clipboard.setData(
                      ClipboardData(text: _createdPassword!),
                    ),
                  ),
                ],
              ),
            ...
            children: [
              ...
              rolesAsync.when(
                ...
                data: (roles) => DropdownButtonFormField<String>(
                  value: roles.any((r) => r.code == _roleCode)
                      ? _roleCode
                      : roles.first.code,
                  decoration: const InputDecoration(
                    labelText: 'Role',
              ...
              unlinkedAsync.when(
                ...
                data: (emps) => DropdownButtonFormField<String?>(
                  value: _employeeId,
                  decoration: const InputDecoration(
                    labelText: 'Link to employee (optional)',
                    border: OutlineInputBorder(),
                    isDense: true,
                  ),
                  items: [
                    const DropdownMenuItem<String?>(
                      value: null,
                      child: Text('(none — standalone user)'),
                    ),
                    for (final e in emps)
...
class _SetPasswordDialogState extends ConsumerState<_SetPasswordDialog> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref
          .read(userManagementRepositoryProvider)
          .setPassword(widget.user.userId, pw);
    ...
    if (_newPassword != null) {
      return AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              '${widget.user.email} must change this password on next login. Copy it now — it will not be shown again.',
            ),
            const SizedBox(height: 12),
            Container(
              padding: const EdgeInsets.all(12),
...
class _ChangeRoleDialogState extends ConsumerState<_ChangeRoleDialog> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref
          .read(userManagementRepositoryProvider)
          .updateRole(widget.user.userId, _selected);
...
class _LinkEmployeeDialog extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_LinkEmployeeDialog> createState() =>
      _LinkEmployeeDialogState();
}
...
class _LinkEmployeeDialogState extends ConsumerState<_LinkEmployeeDialog> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref
          .read(userManagementRepositoryProvider)
          .linkEmployee(widget.user.userId, _selected);
      if (mounted) Navigator.pop(context);
    } catch (e) {
      setState(() => _error = e.toString());
    } finally {
